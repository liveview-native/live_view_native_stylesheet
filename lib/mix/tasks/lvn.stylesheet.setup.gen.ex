defmodule Mix.Tasks.Lvn.Stylesheet.Setup.Gen do
  use Mix.Task

  alias Mix.LiveViewNative.Context

  @shortdoc "Setup Generators for LiveView Native Stylesheet within a project"
  ...
  def generators(_context) do
    ...
    |> Map.keys()
    |> Enum.map(fn(key) -> {"lvn.stylesheet.gen", [key, "App"]} end)
  end
  ...
  def validate_args!(_args) do
    Mix.raise("""
    mix lvn.stylesheet.setup.gen does not take any arguments, only the following switches:
